animate_movie <- function(movie_data, color_scheme, filename = "movie.gif") {
  message("Generating anim object...")

  anim <- ggplot2::ggplot(dplyr::arrange(movie_data, depth), ggplot2::aes(x = x, y = y)) +
    ggplot2::geom_point(ggplot2::aes(size = depth, color = Cell_Type)) +
    ggplot2::coord_fixed() + 
    ggplot2::scale_color_manual(values = color_scheme) +
    ggplot2::scale_size_continuous(range = c(0.02, 0.1), guide = "none") +
